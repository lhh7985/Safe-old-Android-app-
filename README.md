# Safe-old-Android-app
## 1. 낙상 감지 및 자동 신고 앱

 - 아두이노에서 블루투스로 신호를 받아 알람을 울림
 - 사용자가 알람을 끄지 않을 시 현재 GSP의 주소 정보를 통해 자동 신고


## 2. 프로젝트 정보

    Android Studio, java, googleMap, bluetooth를 사용
    그 외에 아두이노, 블루투스 모듈, 6축 가속도 센서(각속도, 가속도)가 필요합니다.

## 3. 흐름도

   <img witdh="600" src="https://user-images.githubusercontent.com/59523147/121900410-5ade3a80-cd60-11eb-9cf4-740c6625be4e.png">
    
## 4. 사용 방법 (Header 2)

    아두이노 하드웨어 제작이 필요합니다.        
   - 하드웨어 구성도
    <img width="800" src="https://user-images.githubusercontent.com/59523147/121900493-6e89a100-cd60-11eb-9749-11b69229e33e.png">
    
## 5. 실행 화면

 - App 메인 화면 및 알람
<img width="600" src="https://user-images.githubusercontent.com/59523147/121900687-a264c680-cd60-11eb-9e4c-b0d90a3e1718.png">
